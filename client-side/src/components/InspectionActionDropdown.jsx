...
function InspectionActionDropdown({ inspection, onDelete, onEdit, isLoading }) {
  ...
  const handleDelete = () => {
    if (
      window.confirm(
        `Are you sure you want to delete inspection ${inspection.inspectionNo || inspection.id}? This action cannot be undone.`,
      )
    ) {
        ...
        <div className='animate-fade-in absolute right-0 z-50 mt-2 w-48 rounded-md border border-gray-200 bg-white shadow-lg'>
          <div className='py-1'>
            ...
            >
              ...
              {isLoading ? 'Deleting...' : 'Delete Inspection'}
            </button>
          </div>
        </div>
      )}
...
export default InspectionActionDropdown;
