...
function Home() {
  ...
  const [searchField, setSearchField] = useState('id'); // Default to searching by Transformer No.
  const [activeView, setActiveView] = useState('transformers'); // 'transformers' or 'inspection'
  ...
  const filteredTransformers = transformers.filter(transformer => {
    // Search filter - now using the selected search field
  ...
  return (
    <div className="min-h-screen p-8 bg-[#E5E4E2]">
      ...
      {/* Filter Controls - Only show for transformers view */}
      {activeView === 'transformers' && (
        <div className="mb-6 bg-[#F5F5F5] p-4 rounded-lg shadow-md flex flex-wrap items-center gap-4">
          ...
          {/* Reset Button as Centered Text Link with Bold on Hover */}
          <button
      ...
      {activeView === 'transformers' ? (
        ...
        <div className="bg-[#F5F5F5] p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Inspection Dashboard</h2>
          <p className="text-gray-600">Inspection content will be displayed here.</p>
        </div>
      )}
    </div>
  );
